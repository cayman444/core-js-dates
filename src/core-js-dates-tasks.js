...
function getDayName(date) {
  ...
  return dictionary[new Date(date).getDay()];
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.floor(diff / millisecondsInDay) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const startInterval = Date.parse(start);
  const endInterval = Date.parse(end);
  const currentInterval = Date.parse(date);
